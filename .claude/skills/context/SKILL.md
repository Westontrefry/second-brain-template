---
name: context
description: Export the user's compact learning-state summary (brain context) for pasting into any AI assistant, optionally scoped to one track or goal. Use when they ask for their "context", want to brief another AI/chat on what they know, or ask "where do I stand" across goals.
---

# /context — portable learning-state export

1. Run the export (scope it if they named a track or goal):
   - `.venv/bin/python -m brain context`
   - `.venv/bin/python -m brain context --track <slug>` / `--goal <goal-id>`
   Unknown slug? The error lists what's available — show them the list.
...
4. If they ask what to do about a gap, that's /review or /quiz territory —
